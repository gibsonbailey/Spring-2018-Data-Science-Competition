...
class CNNmodel():
    ...
    def __init__(self, model_name, output_length, LEARNING_RATE, dense_layer1_width, activation_function):
        ...
        # Set up optimizer
        if activation_function == "RMS":
            self.optimizer = tf.train.RMSPropOptimizer(learning_rate=LEARNING_RATE).minimize(self.cross_entropy)
        elif activation_function == "SGD":
            self.optimizer = tf.train.GradientDescentOptimizer(learning_rate=LEARNING_RATE).minimize(self.cross_entropy)
        elif activation_function == "Adam":
    ...
    def train(self, sess, EPOCHS, BATCH_SIZE, train_data, train_labels, eval_data, eval_labels):
        ...
        for epoch in range(EPOCHS):
             ...
             for i in range(total_batch_num):
                 ...
                 _, c = sess.run([self.optimizer, self.cross_entropy], feed_dict={self.x: batch_x, self.y: batch_y, self.keep_prob: 0.6})
    ...
    def train(self, sess, EPOCHS, BATCH_SIZE, train_data, train_labels, eval_data, eval_labels):
        ...
        for epoch in range(EPOCHS):
             ...
             for i in range(total_batch_num):
                 ...
                 batch_low_ind += BATCH_SIZE
                 batch_high_ind += BATCH_SIZE
                 _, c = sess.run([self.optimizer, self.cross_entropy], feed_dict={self.x: batch_x, self.y: batch_y, self.keep_prob: 0.6, self.keep_prob_conv: 1.0})
                 avg_cost += c / total_batch_num
